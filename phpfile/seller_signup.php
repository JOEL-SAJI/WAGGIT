...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $user_type = "seller"; // Hardcoded user type for seller registration
    
    // Validate input
    if (empty($username) || empty($password) || empty($confirm_password) || empty($email) || empty($first_name) || empty($last_name)) {
        header("Location: seller_signup.html?error=Please fill in all fields");
        exit();
    }
    
    // Check if passwords match
    if ($password !== $confirm_password) {
        header("Location: seller_signup.html?error=Passwords do not match");
        exit();
    }
    ...
    if ($check_result->num_rows > 0) {
        header("Location: seller_signup.html?error=Username already exists");
        $check_stmt->close();
        exit();
    }
    $check_stmt->close();
    ...
    if ($check_email_result->num_rows > 0) {
        header("Location: seller_signup.html?error=Email already exists");
    ...
    if ($insert_stmt->execute()) {
        ...
        header("Location: seller_login.html?success=Account created successfully! Please login.");
        exit();
    } else {
        // Registration failed
        header("Location: seller_signup.html?error=Registration failed. Please try again.");
        exit();
    }
